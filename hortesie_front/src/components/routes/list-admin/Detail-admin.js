  ...
  return (
    <div className="Detail-container">
      ...
      <div className="button-admin">
        <Button onClick={() => format_new_data_to_save()} sx={{ fontSize: 20 }} endIcon={<SaveIcon sx={{ fontSize: 40 }} />}>
          Sauvegarder
        </Button>
      </div>
      {data &&
        <div className="field-container">
          ...
          <TextField id="nom-projet" className="form-projet" defaultValue={data[0].nom}></TextField>
          <label className="label-detail">Description du projet</label>
          <TextField id="description-projet" className="form-projet" defaultValue={data[0].description} multiline></TextField>

          <label className="label-detail">Ville du projet</label>
          <TextField id="ville-projet" className="form-projet" defaultValue={data[0].ville} multiline></TextField>


          <div className="image-admin-container">
            <div className="image-vignette">
              <img className="vignette-admin" src={vignette}></img>
              <input name="file_vignette" style={{display:'none'}} accept="image/*" ref={hiddenFileInput} type="file" onChange={async (event) => {
                console.log("ezjajez")
                const setving = async ()=>{
              ...
              <Button type='button' className="change-vignette" onClick={handleClick}>Changer la vignette</Button>
            </div>
            <div className="add-photo-button">


              <input multiple name="file" style={{display:'none'}} accept="image/*" ref={hiddenFileInput_photos} type="file" onChange={(event) => {
                const file = event.target.files
                console.log("all file", file)
                setFile(file)
                imageHandler(event)
              }} />
              <Button endIcon={<AddIcon></AddIcon>} type="button" onClick={handleClick_photo} >
                Ajouter des photos
              </Button>
            </div>
            {data.map((elem, i) => {
                ...
                return (
                  ...
                  <div key={i} className="image-container">
                    ...
                    <div className="delete-button-img">
                      <IconButton backgroundColor="error" onClick={() => removeImage(elem.nom)}>
